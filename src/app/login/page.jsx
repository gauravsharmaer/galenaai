...
const Login = () => {
  ...
  return (
    <div>
      <Image src={regimg} alt="pic" width={500} className="px-4 rounded-3xl sm:w-[100rem] sm:h-[30rem]" />
      ...
      <div className="flex-col bg-white shadow-xl mx-8 pl-3 absolute top-[17rem] rounded-lg sm:left-[30%] sm:w-[30%]">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <input
            ...
            required
          />
          <div className="text-center bg-orange-400 mx-20 pt-4 rounded-lg font-bold text-white mb-4 sm:pl-3">
            <button className="mb-5">SIGN IN</button>
          </div>
          {/* Display error message */}
          {session.error && <p>{session.error}</p>}
        </form>
        <div>
          <button onClick={() => signIn("google")} className="pl-16 absolute sm:left-20 hover:text-orange-400">
            Login with Google <span className="text-orange-400">  <GoogleIcon></GoogleIcon></span>
          </button>
        </div>
        <br />
